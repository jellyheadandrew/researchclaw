...
def _create_run_exp_sh(trial_dir: Path, script_body: str = "") -> Path:
    """Create a run_exp.sh script in the trial directory."""
...
# --- Tests for _save_output_log ---


class TestSaveOutputLog:
    def test_creates_log_file(self, tmp_path: Path) -> None:
        trial_dir = tmp_path / "trial"
        (trial_dir / "experiment" / "outputs").mkdir(parents=True)
        log_path = _save_output_log(trial_dir, 0, "some output")
        assert log_path.exists()
        assert log_path.name == "log_iter000"

    def test_log_content(self, tmp_path: Path) -> None:
        trial_dir = tmp_path / "trial"
        (trial_dir / "experiment" / "outputs").mkdir(parents=True)
        log_path = _save_output_log(trial_dir, 0, "line1\nline2\n")
        assert log_path.read_text() == "line1\nline2\n"

    def test_retry_numbering(self, tmp_path: Path) -> None:
        trial_dir = tmp_path / "trial"
        (trial_dir / "experiment" / "outputs").mkdir(parents=True)
        log0 = _save_output_log(trial_dir, 0, "attempt 0")
        log1 = _save_output_log(trial_dir, 1, "attempt 1")
        log2 = _save_output_log(trial_dir, 2, "attempt 2")
        assert log0.name == "log_iter000"
        assert log1.name == "log_iter001"
        assert log2.name == "log_iter002"

    def test_creates_outputs_dir_if_missing(self, tmp_path: Path) -> None:
        trial_dir = tmp_path / "trial"
        trial_dir.mkdir()
        log_path = _save_output_log(trial_dir, 0, "output")
        assert log_path.exists()
        assert (trial_dir / "experiment" / "outputs").is_dir()


# --- Tests for _run_experiment_subprocess ---


class TestRunExperimentSubprocess:
    def test_success_returns_zero(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, 'echo "hello world"')
        config = ResearchClawConfig()
        exit_code, output = _run_experiment_subprocess(trial_dir, config, None)
        assert exit_code == 0
        assert "hello world" in output

    def test_failure_returns_nonzero(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, "exit 42")
        config = ResearchClawConfig()
        exit_code, output = _run_experiment_subprocess(trial_dir, config, None)
        assert exit_code == 42
    ...
    def test_streams_output_to_chat(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, 'echo "line1"\necho "line2"')
        config = ResearchClawConfig()
        chat = FakeChat()
        exit_code, output = _run_experiment_subprocess(trial_dir, config, chat)
        assert exit_code == 0
        assert any("line1" in m for m in chat.sent)
        assert any("line2" in m for m in chat.sent)

    def test_captures_stderr(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, 'echo "stderr msg" >&2')
        config = ResearchClawConfig()
        exit_code, output = _run_experiment_subprocess(trial_dir, config, None)
    ...
    def test_timeout_kills_process(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, "sleep 60")
...
# --- Tests for handle_experiment_execute ---


class TestHandleExperimentExecute:
    def test_success_returns_eval_implement(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, 'echo "done"')
        meta = TrialMeta()
        config = ResearchClawConfig()
        chat = FakeChat()
    ...
    def test_failure_returns_experiment_implement(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
    ...
    def test_failure_increments_retry_count(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, "exit 1")
        meta = TrialMeta(experiment_retry_count=0)
        config = ResearchClawConfig(max_retries=5)
        handle_experiment_execute(trial_dir, meta, config, None)
    ...
    def test_max_retries_message(self, tmp_path: Path) -> None:
        ...
        config = ResearchClawConfig(max_retries=5)
        chat = FakeChat()
        handle_experiment_execute(trial_dir, meta, config, chat)
        assert any("Max retries" in m for m in chat.sent)

    def test_creates_output_log(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, 'echo "experiment output"')
        meta = TrialMeta()
        config = ResearchClawConfig()
        handle_experiment_execute(trial_dir, meta, config, None)
        log = trial_dir / "experiment" / "outputs" / "log_iter000"
        assert log.exists()
        assert "experiment output" in log.read_text()

    def test_retry_creates_sequential_logs(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, "exit 1")
        meta = TrialMeta(experiment_retry_count=2)
        config = ResearchClawConfig(max_retries=5)
        handle_experiment_execute(trial_dir, meta, config, None)
        assert (trial_dir / "experiment" / "outputs" / "log_iter002").exists()
    ...
    def test_sends_status_messages(self, tmp_path: Path) -> None:
        ...
        chat = FakeChat()
        handle_experiment_execute(trial_dir, meta, config, chat)
        assert any("EXPERIMENT_EXECUTE" in m for m in chat.sent)
    ...
    def test_failure_saves_exit_code(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, "exit 3")
        meta = TrialMeta()
        config = ResearchClawConfig(max_retries=5)
        handle_experiment_execute(trial_dir, meta, config, None)
        assert meta.experiment_exit_code == 3

    def test_retry_message_on_failure(self, tmp_path: Path) -> None:
        trial_dir = _setup_sandbox(tmp_path)
        _create_run_exp_sh(trial_dir, "exit 1")
        meta = TrialMeta(experiment_retry_count=1)
        config = ResearchClawConfig(max_retries=5)
        chat = FakeChat()
        handle_experiment_execute(trial_dir, meta, config, chat)
    ...
    def test_attempt_number_in_message(self, tmp_path: Path) -> None:
        ...
        chat = FakeChat()
        handle_experiment_execute(trial_dir, meta, config, chat)
        # Should show attempt 3/5 (retry_count=2 means this is 3rd attempt)
        assert any("attempt 3/5" in m for m in chat.sent)


# --- Tests for TrialMeta retry_count ---


class TestTrialMetaRetryCount:
    def test_default_retry_count_is_zero(self) -> None:
        meta = TrialMeta()
        assert meta.experiment_retry_count == 0

    def test_retry_count_round_trip(self, tmp_path: Path) -> None:
        meta = TrialMeta(experiment_retry_count=3)
        path = tmp_path / "meta.json"
        meta.to_json(path)
        loaded = TrialMeta.from_json(path)
        assert loaded.experiment_retry_count == 3
